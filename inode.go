...
func (xfs *FileSystem) ReadInode(ino uint64) (*Inode, error) {
	...
	offset := int64(xfs.PrimaryAG.SuperBlock.InodeAbsOffset(ino))
	inodeSize := int64(xfs.PrimaryAG.SuperBlock.Inodesize)
	buf := make([]byte, inodeSize)
	if _, err := xfs.dev.ReadAt(buf, offset); err != nil {
		return nil, xerrors.Errorf("failed to read inode at offset %d: %w", offset, err)
	}
...
func (xfs *FileSystem) parseBtreeNode(blockNumber int64, inode Inode) ([]BmbtKey, []BmbtPtr, error) {
	...
	b, err := xfs.readBlock(physicalBlockOffset, 1)
	if err != nil {
...
// parseXDB3Block parses a XDB3 block (single block directory architecture)
...
// parseXDD3Block parses a XDD3 block (multi block directory architecture)
...
func (xfs *FileSystem) parseDir2DataEntry(r io.Reader) ([]Dir2DataEntry, error) {
	...
	for {
		...
		// Dir2DataEntry is 8-byte aligned
...
func (xfs *FileSystem) parseDir2Block(bmbtIrec BmbtIrec) ([]Dir2DataEntry, error) {
	...
	*/
	...
	for blockOffset := bmbtIrec.StartBlock; blockOffset < bmbtIrec.StartBlock+bmbtIrec.BlockCount; blockOffset++ {
		...
		blockData, err := xfs.readBlock(physicalBlockOffset, 1)
		if err != nil {
			return nil, xerrors.Errorf("failed to read block: %w", err)
		}
